...
// mỗi lần rquest
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
		...
		if (authHeader != null && authHeader.startsWith("Bearer ")) {
			...
			try {
				...
				sendUnauthorizedError(response, request, "Token không hợp lệ hoặc đã hết hạn");
				return;
			}
		}

		else {
			sendUnauthorizedError(response, request, "Thiếu Authorization header hoặc không đúng định dạng Bearer");
			return;
		}
	...
	private boolean isWhitelisted(String path) {
		return Stream.of(AuthWhitelist.PATHS).anyMatch(pattern -> new AntPathMatcher().match(pattern, path));
	}

	private void sendUnauthorizedError(HttpServletResponse response, HttpServletRequest request, String message)
			throws IOException {
